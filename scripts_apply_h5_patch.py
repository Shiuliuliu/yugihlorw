...
# Marker 1: line 2335 -> ClientData.sendBuyPackage = function(packageId, count) return true end
# Marker 2: ClientData.sendCardBoxInfo = function(boxId)
...
# Also update isLiya / cost in doBuyPackage:
old_cost_block = """\t\t\tlocal isLiya = (boxId and boxId >= 101001 and boxId <= 135050)
\t\t\tlocal liyaIdx = nil
\t\t\tif isLiya then
...
new_cost_block = """\t\t\tlocal isLiyaOrExtra = (boxId and boxId >= 101001 and boxId <= 140050)
\t\t\tlocal packIdx = nil
...
if old_cost_block in new_h5:
    new_h5 = new_h5.replace(old_cost_block, new_cost_block)
    print("Replaced cost block successfully!")
else:
    print("Warning: old_cost_block not matched directly, checking whitespace")
    # let's try replacing line by line
    import re
    new_h5 = re.sub(
        r'local isLiya = \(boxId and boxId >= 101001 and boxId <= 135050\).*?costVal = \(numPacks >= 50 and 22500\) or \(numPacks >= 10 and 4500\) or \(500 \* numPacks\)\s+end\s+end',
