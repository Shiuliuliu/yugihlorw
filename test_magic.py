...
with open(r'D:\yugitauapk\extracted_bins\magic.bin', 'rb') as f:
    raw = f.read()
...
while ptr < len(raw):
    ...
    if row_count < 10:
        print(f"Spell {row_count}: ID={row['_id']} Name={name} Type={row['_type']} Pinyin={row['_py']}")
    row_count += 1

print(f"Finished magic.bin! Parsed {row_count} spells, remaining bytes: {len(raw)-ptr}")
