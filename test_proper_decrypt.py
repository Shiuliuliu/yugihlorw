...
def decrypt_file(raw_data, key):
    ...
    dec_head = decrypt_data(raw_data[:enc_len], key)
    return dec_head + raw_data[enc_len:]
...
while off < len(data):
    ...
    if name in ['monster.bin', 'magic.bin', 'trap.bin', 'rare.bin']:
        bins[name] = dec_file
...
# Let's test monster.bin
raw = bins['monster.bin']
...
print(f"SUCCESS! Parsed {len(monsters)} monsters to EOF! Remaining bytes: {len(raw)-ptr}")
for m in [monsters[0], monsters[54], monsters[55], monsters[-1]]:
    print(f"  ID: {m['_id']} | Name: {get_text(m['_nameSid'])} | ATK: {m['_atk']} | DEF: {m['_hp']} | Star: {m['_star']} | Pinyin: {m['_py']}")
